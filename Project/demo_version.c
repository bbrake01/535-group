/*
This code was used for the demo video. Our previous code was unable to successfully extract microphone data
in any usable format due to device tree issues, so this code uses a button to simulate the input of an
audio signal into the system. From this the code control flow can be demonstrated.
*/
...
static void config_mcasp(void)
{   
    mcasp_base = ioremap(0x48038, 0x2000);
...
static void timer_callback(struct timer_list *t)
{
	...
	if(distance<THRESHOLD_D)
	{
	mod_timer(&timer, jiffies + msecs_to_jiffies(distance*20));	
	}
	else
	{
	mod_timer(&timer, jiffies + msecs_to_jiffies(500));   
